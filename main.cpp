...
int main()
{
    ...
    if (!font.openFromFile("arial.ttf"))
        return -1; // Asegúrate de tener una fuente
    ...
    while (window.isOpen())
    {
        if (recording)
        {
            ...
            // 1. Calcular velocidad hipotética (igual que en tu JS)
            sf::Vector2f virtualVel = (mousePos - startPos) * 0.8f;

            // 2. Crear un "cuerpo fantasma" para el cálculo
            GravityBody ghost(std::log(startM), startM);
            ghost.pos = startPos;
            ghost.vel = virtualVel;

            // 3. Obtener el cuerpo más masivo como referencia (el "Sol")
            // Suponemos que bodies[0] es el centro de masa principal
            if (!bodies.empty())
            {
                const GravityBody &parent = bodies[0];
                OrbitData previewData = getOrbitPrediction(ghost, parent);

                // 4. Dibujar la elipse de predicción
                drawOrbit(window, parent, previewData);
            }

            // 5. Dibujar la línea de la tangente (vector de velocidad)
            sf::Vertex line[] = {
                sf::Vertex{startPos, sf::Color(200, 240, 80)},
                sf::Vertex{mousePos, sf::Color(200, 240, 80)}};
            window.draw(line, 2, sf::PrimitiveType::Lines);

            // 6. Dibujar el cuerpo que se va a crear
            sf::CircleShape previewBody(std::log(startM));
            previewBody.setOrigin({std::log(startM), std::log(startM)});
            previewBody.setPosition(startPos);
            previewBody.setFillColor(sf::Color(255, 255, 255, 150));
            window.draw(previewBody);
        }
        while (const std::optional event = window.pollEvent())
        {
            ...
            if (const auto *keyPressed = event->getIf<sf::Event::KeyPressed>())
            {
                ...
                if (keyPressed->code == sf::Keyboard::Key::Down)
                    startM = std::max(5.0f, startM - 5.0f);
            }
        ...
        for (size_t i = 0; i < bodies.size(); i++)
        {
            ...
            bodies[i].update();
            if (i != parentI)
                bodies[i].setParent(bodies[parentI]);
        }
